...
//    fprintf("graph.txt", );    
  int numOfVert = numVerts(gi->graph);// Get num of verts
  int *temp = (int*) malloc(numOfVert * sizeof(int));
  int j = 0;

    for(int i = 0; i < numOfVert; i++)
    {
        temp = successors(gi -> graph, i); //Get the sucessors
        //int j = 0;  //A counter
      
        while(temp[j] != -1)
        {
            printf(" %s", gi->vertnames[i]); //Should be the vert
            printf(" %s", gi->vertnames[temp[j]]); //Should be the Succ
            printf("(%.2f) \n" ,edge(gi->graph, i, temp[j]));//Should be the weight;
            temp[j] = -1;
            j++;
        }
        j = 0;
